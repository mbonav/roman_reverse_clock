...
def test_roman_to_integer():
    ...
    assert roman_to_integer("IXX") == (
        None,
        "Invalid Roman numeral: Invalid subtractive group",
    )
    assert roman_to_integer("MMVIIZ") == (
        None,
        "Invalid Roman numeral: Unknown character 'Z'",
    )
...
def test_angle_between_hands():
    ...
    assert angle_between_hands(27, 30) == (
        None,
        "Invalid time: Time values are out of range",
    )
